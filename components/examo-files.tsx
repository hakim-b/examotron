  ...
  const fetchFolderContentsRecursive = async (
    ...
    const subFolders = await Promise.all(
      result.prefixes.map(async (subFolderRef) => {
        const subFolderContents = await fetchFolderContentsRecursive(
          subFolderRef
        );
        return {
  ...
  return (
    <>
      <Tabs defaultValue="biol" className="mt-5 w-full">
        <TabsList className="grid w-full grid-cols-5 md:grid-cols-12 h-28 md:h-10">
